...
def process_token(token, previous_token):
    ...
    if token == TOKEN_NEWPAGE and previous_token == TOKEN_NEWLINE:
        ...
        if token.isalpha() and token.isupper():
            prefix = [{'box': alphabet.uppercase_prefix, 'desc': alphabet.uppercase_description}, {'box': alphabet.uppercase_prefix, 'desc': alphabet.uppercase_description}]
        else:
...
if __name__ == "__main__":
    ...
    text = 'Esto es el Ñandú número 20 en la lista. MAYUSCULA Mayuscula Mayuscula2'
