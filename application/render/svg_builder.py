...
def render_box(box_dict, box_x_offset, box_y_offset, box_height, box_width, text_height_proportion, write_original_text):
    ...
    if write_original_text:
        ...
        output += '<text x="' + str(box_x_offset + round(box_width / 4) ) + '" y="' + str(box_y_offset + round(box_height * 0.9)) + '" font-size="' + str(round(box_height * text_height_proportion )) + '">' + decoded_box_desc + '</text>'

    return output

def render_pages(box_list, box_height, boxes_per_row, rows_per_page, left_margin, top_margin, text_height_percentage, write_original_text=True):

    text_height_proportion = text_height_percentage / 100
    ...
    for box_dict in box_list:
        ...
        if box_string not in (NEWLINE_BOX,NEWPAGE_BOX):
            page_output += render_box(box_dict, box_x_offset, box_y_offset, box_height, box_width, text_height_proportion, write_original_text)
